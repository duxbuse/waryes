...
/**
 * Minimal unit interface for checksum computation.
 * Both client Unit and server SimUnit satisfy this contract.
 */
export interface ChecksumUnit {
  id: string;
  health: number;
  morale: number;
  suppression: number;
 ...
 */
export function computeGameStateChecksum(units: readonly ChecksumUnit[]): number {
  let hash = 0;
 ...
 */
export function computePositionChecksum(units: readonly ChecksumUnit[]): number {
  let hash = 0;
